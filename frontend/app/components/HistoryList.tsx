...
export default function HistoryList({ items }: HistoryListProps) {
  return (
    // One card holding all rows rather than a card per analysis: a long list of
    // gradient-bordered cards stops being scannable, and dividers inside a
    // single section read the same way ScoreCard and SkillsPanel group content.
    <SectionCard eyebrow="History">
      <ul className="flex flex-col">
        {items.map((item) => (
          <li key={item.id} className="border-b border-border last:border-b-0">
            {/* Negative margin lets the hover fill bleed past the text without
                shifting the row's alignment with the card's padding. */}
            <Link
            ...
            >
              ...
              {/* Plain mono, not ScoreCard's indigo→violet gradient — that
                  treatment is for the one headline number on a page, not for
                  every row of a list. */}
